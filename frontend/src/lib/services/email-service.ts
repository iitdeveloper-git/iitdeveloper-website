...
export class EmailService {
  private static fromEmail = process.env.FROM_EMAIL || 'info@iitdeveloper.com';
  ...
  // ── Raw send (supports attachments, custom headers, etc.) ────
  static async sendRaw(options: {
    to: string;
    subject: string;
    html: string;
    replyTo?: string;
    ...
    try {
      ...
      console.log(`✅ Raw email sent to ${options.to} — messageId: ${info.messageId}`);
  ...
  // ── Contact form — internal alert to sales team ───────────────
  ...
  private static async logEmail(data: {
    ...
    try {
      ...
      // Non-fatal — don't break the send flow
    }
  }
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f9fafb;">
  ...
  /** Contact confirmation email — sent to the customer */
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  ...
  /** Internal notification email — sent to the sales team */
